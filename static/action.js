...
const SDApi = {
    ...
    common: {
        ...
        setSettings: function (context, payload) {
            SDApi.send($SD.uuid, 'setSettings', {
                payload: payload
            });
        },
...
/* 指定国际化 */
function localUI(key, value) {
    return $localizedStrings[key][value]
}

/* 国际化所有字符串 */
function localAll(isLog) {
    Array.from($('.sdpi-wrapper').getElementsByTagName('*')).forEach(item => {
        /* 普通文字过滤 */
        if (item.innerText.trim() &&
            item.className !== 'sdpi-item' && item.className !== 'sdpi-item-child' &&
            item.tagName !== 'SELECT' && item.tagName !== 'LABEL') {
            isLog && console.log(item.innerText);
            item.innerText = $localizedStrings[item.innerText]
        }
        /* 输入框提示词过滤 */
        if (item.tagName === 'INPUT' || item.tagName === 'TEXTAREA') {
            isLog && console.log(item.placeholder);
            item.placeholder = $localizedStrings[item.placeholder]
        }
    })
}